...
def test_question_browser_search_same_label_matches_reasoning_names_and_local_alias_snapshots(client):
    db_gen = client.app.dependency_overrides[get_db]()
    db = next(db_gen)
    try:
        source_opus = make_model(
            db,
            name="Claude Opus 4.6 [Reasoning (high)]",
            provider=ProviderType.anthropic,
            base_url="https://api.anthropic.com/v1/messages",
        ...
        source_oss.model_format = "GGUF"
        source_oss.quantization = "MXFP4"

        candidate_opus = make_model(
            db,
            name="Claude Opus 4.6 [Reasoning (high)]",
            provider=ProviderType.anthropic,
            base_url="https://api.anthropic.com/v1/messages",
            model_id="claude-opus-4-6-candidate",
            is_reasoning=1,
            reasoning_level=ReasoningLevel.high,
        )
        candidate_oss = make_model(
            db,
            name="GPT-OSS 120B",
            provider=ProviderType.lmstudio,
            base_url="http://localhost:1234/v1/chat/completions",
            model_id="openai/gpt-oss-120b-candidate",
            is_reasoning=1,
            reasoning_level=None,
        )
        candidate_oss.model_format = "GGUF"
        candidate_oss.quantization = "MXFP4"
        judge = make_model(db, name="Judge", provider=ProviderType.google, base_url="http://google.local", model_id="judge-1")

        source_run = make_completed_run(
            db,
            name="Source Run",
            model_presets=[source_opus, source_oss],
            judge_presets=[judge],
        )
        source_question = make_question_with_generations(
            db,
            ...
            order=0,
            prompt="Source same-label question",
            generation_models=[source_opus, source_oss],
        )

        candidate_snapshot = _build_snapshot([candidate_opus, candidate_oss], [judge], model_overrides={
            candidate_oss.id: {
                "base_url": "http://cachy.local:1234/v1/chat/completions",
            },
        })
        candidate_snapshot["models"][1].pop("model_format", None)
        candidate_snapshot["models"][1].pop("quantization", None)
        candidate_snapshot["models"][1].pop("is_reasoning", None)
        candidate_snapshot["models"][1].pop("reasoning_level", None)

        candidate_run = make_completed_run(
            db,
            name="Candidate Run",
            model_presets=[candidate_opus, candidate_oss],
            judge_presets=[judge],
            run_config_snapshot=candidate_snapshot,
        )
        candidate_question = make_question_with_generations(
            db,
            run=candidate_run,
            order=0,
            prompt="Candidate same-label question",
            generation_models=[candidate_opus, candidate_oss],
        )
        db.commit()
        ...
        resp = client.get(
            "/api/question-browser/search",
            params={
                "models": f"{source_opus.id},{source_oss.id}",
                "match": "same-label",
                "sourceRun": source_run.id,
                "sourceQuestion": source_question.id,
                "limit": 20,
        ...
        assert candidate_question.id in {row["question_id"] for row in body["rows"]}
        assert [model["resolved_label"] for model in body["selected_models"]] == [
            "Claude Opus 4.6 [Reasoning (high)] (@ api.anthropic.com)",
            "GPT-OSS 120B (@ localhost:1234)",
        ]

        standalone_resp = client.get(
            "/api/question-browser/search",
            params={
                "models": f"{source_opus.id},{source_oss.id}",
                "match": "same-label",
        ...
        assert standalone_resp.status_code == 200, standalone_resp.text
        standalone_body = standalone_resp.json()
        assert candidate_question.id in {row["question_id"] for row in standalone_body["rows"]}
        assert [model["resolved_label"] for model in standalone_body["selected_models"]] == [
            "Claude Opus 4.6 [Reasoning (high)] (@ api.anthropic.com)",
            "GPT-OSS 120B (GGUF MXFP4 @ localhost:1234)",
        ]
    finally:
        db_gen.close()


def test_question_browser_picker_guidance_collapses_snapshot_variants_into_one_visible_candidate(client):
    db_gen = client.app.dependency_overrides[get_db]()
    db = next(db_gen)
    try:
        candidate_model = make_model(
        ...
        candidate_model.model_format = "GGUF"
        candidate_model.quantization = "MXFP4"
        judge = make_model(db, name="Judge", provider=ProviderType.google, base_url="http://google.local", model_id="judge-1")

        run_one = make_completed_run(
            db,
            name="Run One",
            model_presets=[candidate_model],
            judge_presets=[judge],
            run_config_snapshot=_build_snapshot([candidate_model], [judge], model_overrides={
                candidate_model.id: {
                    "base_url": "http://cachy.local:1234/v1/chat/completions",
                },
            }),
        )
        run_one.run_config_snapshot["models"][0].pop("model_format", None)
        run_one.run_config_snapshot["models"][0].pop("quantization", None)
        run_one.run_config_snapshot["models"][0].pop("is_reasoning", None)
        run_one.run_config_snapshot["models"][0].pop("reasoning_level", None)
        ...
        run_three = make_completed_run(
            db,
            name="Run Three",
            model_presets=[candidate_model],
            judge_presets=[judge],
        )
        run_three.run_config_snapshot["models"][0]["base_url"] = "http://cachy.local:1234/v1/chat/completions"

        make_question_with_generations(db, run=run_one, order=0, prompt="Question one", generation_models=[candidate_model])
        make_question_with_generations(db, run=run_two, order=0, prompt="Question two", generation_models=[candidate_model])
        make_question_with_generations(db, run=run_three, order=0, prompt="Question three", generation_models=[candidate_model])
        db.commit()

        resp = client.get('/api/question-browser/picker-guidance', params={"selected_model_ids": "", "frequency_band": "all"})

        assert resp.status_code == 200, resp.text
        rows = [candidate for candidate in resp.json()["candidates"] if candidate["resolved_label"] == "GPT-OSS 120B"]
        assert len(rows) == 1
        assert rows[0]["active_benchmark_count"] == 3
        assert rows[0]["host_label"] == "localhost:1234"
    finally:
        db_gen.close()
